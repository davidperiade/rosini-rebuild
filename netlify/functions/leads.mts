...
export default async(req:Request,c:Context)=>{const store=getStore(STORE);
 if(req.method==='POST'){const b=await req.json().catch(()=>null);if(!b?.name||!b?.phone||!b?.product||b?.consent!==true)return json({error:'Completează câmpurile obligatorii și acordul privind datele.'},400);const now=new Date().toISOString(),id=await nextId(store);const lead={id,name:String(b.name).trim(),phone:String(b.phone).trim(),product:String(b.product).trim(),message:String(b.message??'').trim(),source:'Site Rosini',createdAt:now,status:'Lead nou',saleValue:0,commissionRate:0,commission:0,notes:'',history:[{at:now,status:'Lead nou'}]};await store.setJSON(id,lead);return json({id,leadId:id},201)}
