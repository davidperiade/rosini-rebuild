...
async function currentAdmin(context){
  ...
  const roles=[...(Array.isArray(u.app_metadata?.roles)?u.app_metadata.roles:[]),...(Array.isArray(u.appMetadata?.roles)?u.appMetadata.roles:[]),...(Array.isArray(u.roles)?u.roles:[])];
...
export default async (req,context)=>{const s=store();try{
if(req.method==='POST'){const b=await req.json(),name=clean(b.name,100),phone=clean(b.phone,25),product=clean(b.product,180),message=clean(b.message,1000);if(name.length<2||!/^[+0-9 ()-]{7,25}$/.test(phone)||product.length<2||b.consent!==true)return json({error:'Completează datele obligatorii și confirmă acordul privind prelucrarea datelor.'},400);let saved=false,lead;for(let attempt=0;attempt<5&&!saved;attempt++){const id=await nextLeadId(),createdAt=new Date().toISOString();lead={id,name,phone,product,message,source:'Site Rosini',status:'Lead nou',saleValue:0,commissionRate:0,commission:0,notes:'',createdAt,history:[{at:createdAt,status:'Lead nou',note:'Lead creat prin formularul Site Rosini.'}]};const result=await s.setJSON(`leads/${id}`,lead,{onlyIfNew:true});saved=!!result.modified}if(!saved)return json({error:'Nu am putut genera un cod unic. Încearcă din nou.'},503);return json({ok:true,leadId:lead.id})}
const admin=await currentAdmin(context);if(!admin)return json({error:'Forbidden'},403);
